...
public class ListarAsistenciaAEventoUseCase(IRepositorioReserva repositorioReserva, IRepositorioEventoDeportivo repositorioEventoDeportivo, IRepositorioPersona repositorioPersona)
{
    public List<Persona> Ejecutar(int idEvento)
    {
        ...
        if (evento is null)
        {
            throw new RepositorioException("El evento no existe.");
        }
        if (evento.FechaHoraInicio < DateTime.Now)
        {
            throw new RepositorioException("Solo puede consultar la asistencia de eventos pasados.");
        }
        foreach (Reserva r in reservas) //itero sobre las reservas recuperando las personas que asistieron al evento
        {
            ...
            {
                Persona? persona = repositorioPersona.PersonaPorId(r.PersonaId); // cambiar nombre del metodo a ObtenerPersona o algo asi
                if (persona is null)
                {
                    throw new RepositorioException("La persona no existe.");
                }
